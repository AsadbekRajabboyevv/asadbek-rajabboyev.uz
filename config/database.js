...
CREATE TABLE IF NOT EXISTS themes (
  ...
  glass_bg TEXT NOT NULL,
  glass_border TEXT NOT NULL,
  is_custom INTEGER DEFAULT 0
);
`);

// Seed default themes
const hasThemes = db.prepare('SELECT count(*) as count FROM themes').get();
if (hasThemes.count === 0) {
    const stmtTheme = db.prepare('INSERT INTO themes (code, name, bg_color, fg_color, accent_color, glass_bg, glass_border, is_custom) VALUES (?, ?, ?, ?, ?, ?, ?, ?)');
    stmtTheme.run('cyber_dark', '🌌 Cyber Midnight Dark', '#00072d', '#f8fafc', '#3b82f6', 'rgba(5, 22, 80, 0.45)', 'rgba(147, 197, 253, 0.15)', 0);
    stmtTheme.run('neon_violet', '💜 Neon Violet Futuristic', '#1D0636', '#F9F5FF', '#C084FC', 'rgba(76, 29, 149, 0.5)', 'rgba(216, 180, 254, 0.3)', 0);
    stmtTheme.run('emerald_matrix', '🟢 Emerald Matrix Cyberpunk', '#022C1A', '#F0FDF4', '#34D399', 'rgba(6, 95, 70, 0.5)', 'rgba(110, 231, 183, 0.3)', 0);
    stmtTheme.run('luxury_gold', '✨ Luxury Gold Obsidian', '#181510', '#FFFBEB', '#FBBF24', 'rgba(120, 85, 20, 0.5)', 'rgba(252, 211, 77, 0.3)', 0);
    stmtTheme.run('sunset_crimson', '🔥 Sunset Crimson Horizon', '#2B0914', '#FFF1F2', '#F43F5E', 'rgba(159, 18, 57, 0.5)', 'rgba(251, 113, 133, 0.3)', 0);
