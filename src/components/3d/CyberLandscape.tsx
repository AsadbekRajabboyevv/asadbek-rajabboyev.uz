'use client';

import React, { useRef, useMemo } from 'react';
import { useFrame } from '@react-three/fiber';
import * as THREE from 'three';

export function CyberLandscape() {
  const terrainMeshRef = useRef<THREE.Mesh>(null);
  const materialRef = useRef<THREE.MeshStandardMaterial>(null);
  const crystalMeshRef = useRef<THREE.Mesh>(null);
  const crystalMaterialRef = useRef<THREE.ShaderMaterial>(null);
  const tubeMeshRef = useRef<THREE.Mesh>(null);
  const tubeMaterialRef = useRef<THREE.ShaderMaterial>(null);

  // High-resolution Plane Geometry for mouse ripples
  const terrainGeometry = useMemo(() => {
    return new THREE.PlaneGeometry(36, 24, 60, 45);
  }, []);

  // Advanced 3D Crystal Shader with noise-based animation
  const crystalShader = useMemo(() => {
    return {
      uniforms: {
        uTime: { value: 0 },
        uColor: { value: new THREE.Color('#3b82f6') },
        uColor2: { value: new THREE.Color('#8b5cf6') },
        uColor3: { value: new THREE.Color('#06b6d4') }
      },
      vertexShader: `
        varying vec2 vUv;
        varying vec3 vPosition;
        varying vec3 vNormal;
        varying float vNoise;
        uniform float uTime;
        
        // Simplex noise function
        vec3 mod289(vec3 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
        vec4 mod289(vec4 x) { return x - floor(x * (1.0 / 289.0)) * 289.0; }
        vec4 permute(vec4 x) { return mod289(((x*34.0)+1.0)*x); }
        vec4 taylorInvSqrt(vec4 r) { return 1.79284291400159 - 0.85373472095314 * r; }
        
        float snoise(vec3 v) {
          const vec2 C = vec2(1.0/6.0, 1.0/3.0);
          const vec4 D = vec4(0.0, 0.5, 1.0, 2.0);
          
          vec3 i  = floor(v + dot(v, C.yyy));
          vec3 x0 = v - i + dot(i, C.xxx);
          
          vec3 g = step(x0.yzx, x0.xyz);
          vec3 l = 1.0 - g;
          vec3 i1 = min(g.xyz, l.zxy);
          vec3 i2 = max(g.xyz, l.zxy);
          
          vec3 x1 = x0 - i1 + C.xxx;
          vec3 x2 = x0 - i2 + C.yyy;
          vec3 x3 = x0 - D.yyy;
          
          i = mod289(i);
          vec4 p = permute(permute(permute(
            i.z + vec4(0.0, i1.z, i2.z, 1.0))
            + i.y + vec4(0.0, i1.y, i2.y, 1.0))
            + i.x + vec4(0.0, i1.x, i2.x, 1.0));
            
          float n_ = 0.142857142857;
          vec3 ns = n_ * D.wyz - D.xzx;
          
          vec4 j = p - 49.0 * floor(p * ns.z * ns.z);
          
          vec4 x_ = floor(j * ns.z);
          vec4 y_ = floor(j - 7.0 * x_);
          
          vec4 x = x_ *ns.x + ns.yyyy;
          vec4 y = y_ *ns.x + ns.yyyy;
          vec4 h = 1.0 - abs(x) - abs(y);
          
          vec4 b0 = vec4(x.xy, y.xy);
          vec4 b1 = vec4(x.zw, y.zw);
          
          vec4 s0 = floor(b0)*2.0 + 1.0;
          vec4 s1 = floor(b1)*2.0 + 1.0;
          vec4 sh = -step(h, vec4(0.0));
          
          vec4 a0 = b0.xzyw + s0.xzyw*sh.xxyy;
          vec4 a1 = b1.xzyw + s1.xzyw*sh.zzww;
          
          vec3 p0 = vec3(a0.xy, h.x);
          vec3 p1 = vec3(a0.zw, h.y);
          vec3 p2 = vec3(a1.xy, h.z);
          vec3 p3 = vec3(a1.zw, h.w);
          
          vec4 norm = taylorInvSqrt(vec4(dot(p0,p0), dot(p1,p1), dot(p2,p2), dot(p3,p3)));
          p0 *= norm.x;
          p1 *= norm.y;
          p2 *= norm.z;
          p3 *= norm.w;
          
          vec4 m = max(0.6 - vec4(dot(x0,x0), dot(x1,x1), dot(x2,x2), dot(x3,x3)), 0.0);
          m = m * m;
          return 42.0 * dot(m*m, vec4(dot(p0,x0), dot(p1,x1), dot(p2,x2), dot(p3,x3)));
        }
        
        void main() {
          vUv = uv;
          vNormal = normal;
          
          vec3 pos = position;
          
          // Multi-layered noise for organic crystal formation
          float noise1 = snoise(pos * 2.0 + uTime * 0.3);
          float noise2 = snoise(pos * 4.0 - uTime * 0.5) * 0.5;
          float noise3 = snoise(pos * 8.0 + uTime * 0.7) * 0.25;
          
          vNoise = noise1 + noise2 + noise3;
          
          // Displace vertices based on noise
          pos += normal * vNoise * 0.3;
          vPosition = pos;
          
          gl_Position = projectionMatrix * modelViewMatrix * vec4(pos, 1.0);
        }
      `,
      fragmentShader: `
        uniform float uTime;
        uniform vec3 uColor;
        uniform vec3 uColor2;
        uniform vec3 uColor3;
        varying vec2 vUv;
        varying vec3 vPosition;
        varying vec3 vNormal;
        varying float vNoise;
        
        void main() {
          // Create iridescent color effect
          vec3 color1 = uColor;
          vec3 color2 = uColor2;
          vec3 color3 = uColor3;
          
          // Mix colors based on noise and position
          float mixFactor1 = (vNoise + 1.0) * 0.5;
          float mixFactor2 = sin(vPosition.y * 3.0 + uTime) * 0.5 + 0.5;
          float mixFactor3 = cos(vPosition.x * 2.0 + uTime * 0.8) * 0.5 + 0.5;
          
          vec3 finalColor = mix(color1, color2, mixFactor1);
          finalColor = mix(finalColor, color3, mixFactor2 * mixFactor3);
          
          // Add fresnel effect for edge glow
          vec3 viewDirection = normalize(cameraPosition - vPosition);
          float fresnel = pow(1.0 - abs(dot(viewDirection, vNormal)), 3.0);
          finalColor += fresnel * 0.5;
          
          // Add pulsing glow
          float pulse = sin(uTime * 2.0) * 0.5 + 0.5;
          finalColor += pulse * 0.2;
          
          // Transparency based on noise
          float alpha = 0.6 + vNoise * 0.3;
          
          gl_FragColor = vec4(finalColor, alpha);
        }
      `
    };
  }, []);

  // Animated DNA helix tube shader
  const tubeShader = useMemo(() => {
    return {
      uniforms: {
        uTime: { value: 0 },
        uColor: { value: new THREE.Color('#3b82f6') },
        uColor2: { value: new THREE.Color('#a855f7') }
      },
      vertexShader: `
        varying vec2 vUv;
        varying vec3 vPosition;
        uniform float uTime;
        
        void main() {
          vUv = uv;
          vPosition = position;
          
          vec3 pos = position;
          
          // DNA helix twist animation
          float twist = sin(pos.y * 3.0 + uTime * 2.0) * 0.3;
          pos.x += twist;
          pos.z += cos(pos.y * 3.0 + uTime * 2.0) * 0.3;
          
          gl_Position = projectionMatrix * modelViewMatrix * vec4(pos, 1.0);
        }
      `,
      fragmentShader: `
        uniform float uTime;
        uniform vec3 uColor;
        uniform vec3 uColor2;
        varying vec2 vUv;
        varying vec3 vPosition;
        
        void main() {
          // Helical color pattern
          float helixPattern = sin(vPosition.y * 10.0 + uTime * 3.0);
          vec3 finalColor = mix(uColor, uColor2, helixPattern * 0.5 + 0.5);
          
          // Add energy flow effect
          float energy = sin(vUv.x * 20.0 - uTime * 5.0) * 0.5 + 0.5;
          finalColor += energy * 0.3;
          
          // Pulsing glow
          float pulse = sin(uTime * 3.0) * 0.5 + 0.5;
          finalColor += pulse * 0.2;
          
          gl_FragColor = vec4(finalColor, 0.8);
        }
      `
    };
  }, []);

  const crystalUniforms = useMemo(() => {
    return {
      uTime: { value: 0 },
      uColor: { value: new THREE.Color('#3b82f6') },
      uColor2: { value: new THREE.Color('#8b5cf6') },
      uColor3: { value: new THREE.Color('#06b6d4') }
    };
  }, []);

  const tubeUniforms = useMemo(() => {
    return {
      uTime: { value: 0 },
      uColor: { value: new THREE.Color('#3b82f6') },
      uColor2: { value: new THREE.Color('#a855f7') }
    };
  }, []);

  useFrame((state) => {
    const time = state.clock.getElapsedTime();
    const mouseX = state.pointer.x * 12;
    const mouseY = state.pointer.y * 8;

    // Update crystal shader
    if (crystalMaterialRef.current) {
      crystalMaterialRef.current.uniforms.uTime.value = time;
      
      if (typeof window !== 'undefined') {
        const accentColor = getComputedStyle(document.documentElement).getPropertyValue('--accent').trim() || '#3b82f6';
        crystalMaterialRef.current.uniforms.uColor.value.setStyle(accentColor);
      }
    }

    // Update tube shader
    if (tubeMaterialRef.current) {
      tubeMaterialRef.current.uniforms.uTime.value = time;
      
      if (typeof window !== 'undefined') {
        const accentColor = getComputedStyle(document.documentElement).getPropertyValue('--accent').trim() || '#3b82f6';
        tubeMaterialRef.current.uniforms.uColor.value.setStyle(accentColor);
      }
    }

    // Mouse-ONLY Ripple Effect on Landscape Grid
    if (terrainMeshRef.current) {
      const geo = terrainMeshRef.current.geometry;
      const pos = geo.attributes.position;

      for (let i = 0; i < pos.count; i++) {
        const vx = pos.getX(i);
        const vy = pos.getY(i);

        const dx = vx - mouseX;
        const dy = vy - mouseY;
        const dist = Math.sqrt(dx * dx + dy * dy);

        // Ripple ONLY under mouse radius
        const mouseRipple = Math.exp(-dist * 0.55) * Math.sin(dist * 4.0 - time * 6.0) * 1.15;
        pos.setZ(i, mouseRipple);
      }

      pos.needsUpdate = true;
      geo.computeVertexNormals();
    }

    // Update Emissive Theme Accent Color
    if (materialRef.current && typeof window !== 'undefined') {
      const accentColor = getComputedStyle(document.documentElement).getPropertyValue('--accent').trim() || '#3b82f6';
      materialRef.current.color.setStyle(accentColor);
      materialRef.current.emissive.setStyle(accentColor);
      materialRef.current.emissiveIntensity = 0.25;
    }
  });

  return (
    <group position={[0, -2.4, -6.5]}>
      {/* 3D Animated Crystal with Noise Shader */}
      <mesh 
        ref={crystalMeshRef}
        position={[-1, 4.5, -4]} 
        rotation={[0.1, -0.15, -0.25]}
      >
        <icosahedronGeometry args={[3, 4]} />
        <shaderMaterial
          ref={crystalMaterialRef}
          vertexShader={crystalShader.vertexShader}
          fragmentShader={crystalShader.fragmentShader}
          uniforms={crystalUniforms}
          transparent={true}
          depthWrite={false}
          blending={THREE.AdditiveBlending}
        />
      </mesh>

      {/* DNA Helix Tube Animation */}
      <mesh 
        ref={tubeMeshRef}
        position={[2, 3, -3]}
        rotation={[0.5, 0, 0]}
      >
        <torusGeometry args={[1.5, 0.3, 16, 100]} />
        <shaderMaterial
          ref={tubeMaterialRef}
          vertexShader={tubeShader.vertexShader}
          fragmentShader={tubeShader.fragmentShader}
          uniforms={tubeUniforms}
          transparent={true}
          depthWrite={false}
          blending={THREE.AdditiveBlending}
        />
      </mesh>

      {/* Flat Cyber Grid Mesh - Flexes ONLY under mouse cursor */}
      <mesh
        ref={terrainMeshRef}
        geometry={terrainGeometry}
        rotation={[-Math.PI / 2.1, 0, 0]}
      >
        <meshStandardMaterial
          ref={materialRef}
          color="#3b82f6"
          emissive="#1e3a8a"
          emissiveIntensity={0.25}
          wireframe={true}
          transparent={true}
          opacity={0.55}
        />
      </mesh>

      {/* Additional Animated Geometric Shapes */}
      <mesh position={[-4, 2, -3]}>
        <octahedronGeometry args={[0.8, 0]} />
        <meshBasicMaterial color="#60a5fa" transparent opacity={0.7} wireframe />
      </mesh>
      <mesh position={[4, 4, -4]}>
        <dodecahedronGeometry args={[0.6, 0]} />
        <meshBasicMaterial color="#a855f7" transparent opacity={0.7} wireframe />
      </mesh>
      <mesh position={[0, 5, -5]}>
        <tetrahedronGeometry args={[0.5, 0]} />
        <meshBasicMaterial color="#06b6d4" transparent opacity={0.7} wireframe />
      </mesh>
    </group>
  );
}
